...
# Skip the QML side (would pull qtdeclarative). C++ QScxmlStateMachine is
# enough for state-machine logic; QML integration isn't needed here.
"${PREFIX}/bin/qt-configure-module" .. \
    -- \
    -DCMAKE_INSTALL_PREFIX="${PREFIX}" \
    -DQT_HOST_PATH="${BUILD_PREFIX}" \
    -DFEATURE_scxml_qml=OFF
